class Boid {
  ...
  public void run(ArrayList<Boid> boids) {
    ...
    if (avoidWalls) {
      acceleration.add(Vector.multiply(avoid(new Vector(position.x(), boxHeight, position.z())), 5));
      acceleration.add(Vector.multiply(avoid(new Vector(position.x(), 0, position.z())), 5));
      acceleration.add(Vector.multiply(avoid(new Vector(boxWidth, position.y(), position.z())), 5));
      ...
      acceleration.add(Vector.multiply(avoid(new Vector(position.x(), position.y(), boxDepth)), 5));
    }
    flock(boids);
    move();
    checkBounds();
  ...
  void checkBounds() {
    if (position.x() > boxWidth)
      position.setX(0);
    if (position.x() < 0)
      position.setX(boxWidth);
    if (position.y() > boxHeight)
      position.setY(0);
    if (position.y() < 0)
      position.setY(boxHeight);
    if (position.z() > boxDepth)
      position.setZ(0);
    if (position.z() < 0)
      position.setZ(boxDepth);
  }
